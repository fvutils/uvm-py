...
class uvm_cmdline_processor (uvm_report_object):
    ...
    @staticmethod
    def get_inst():
        global uvm_cmdline_proc
        if uvm_cmdline_processor.m_inst is None:
            uvm_cmdline_processor.m_inst = uvm_cmdline_processor("uvm_cmdline_proc")
            uvm_cmdline_proc = uvm_cmdline_processor.m_inst;
        return uvm_cmdline_processor.m_inst
    ...
    def get_plusargs(self, args):
        args.clear()
        args.extend(self.m_plus_argv)
    ...
    def get_uvm_args(self, args):
        args.clear()
        args.extend(self.m_uvm_argv)
...
#     bit match_is_regex = (match.len() > 2)  and  (match[0] == "/")  and  (match[match.len()-1] == "/");
#     int len = match.len();
# 
#     args.clear();
#     foreach (m_argv[i]) begin
#       if ( match_is_regex  and  uvm_is_match( match, m_argv[i] ) ) begin
#         args.append( m_argv[i] );
#       end
#       elif:((m_argv[i].len() >= len)  and  (m_argv[i].substr(0,len - 1) == match)) begin
#         args.append(m_argv[i]);
#       end
#     end
# 
...
#         m_argv.append(s);
#         if(s[0] == "+") begin
#           m_plus_argv.append(s);
#         end 
#         if(s.len() >= 4  and  (s[0]=="-"  or  s[0]=="+")) begin
#           sub = s.substr(1,3);
#           sub = sub.toupper();
#           if(sub == "UVM")
#             m_uvm_argv.append(s);
